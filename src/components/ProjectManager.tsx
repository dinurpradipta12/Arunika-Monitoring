...
import { Plus, Database, Server, MoreHorizontal, RefreshCw, Trash2, Edit2, CheckCircle2, AlertTriangle, Loader2, Globe, Shield, Layers } from 'lucide-react';
...
const ProjectManager: React.FC<ProjectManagerProps> = ({ apps, onAddApp, onUpdateApp, onDeleteApp }) => {
  ...
  const [dbType, setDbType] = useState<'postgres' | 'mysql' | 'mongodb' | 'supabase'>('postgres');
  ...
  const [tableName, setTableName] = useState('registrations'); // This is the registration table
  ...
  const handleTestConnection = async () => {
    ...
    if (!projectId) {
      ...
      setTestMessage('Invalid Host. Expected: db.[project-ref].supabase.co');
      setIsTesting(false);
      return;
    }
    ...
    try {
      ...
      const responseReg = await fetch(`${derivedUrl}/rest/v1/${tableName}?select=count&limit=1`, {
        headers: { 'apikey': apiKey, 'Authorization': `Bearer ${apiKey}` }
      });

      // Test 2: Check Users table (Hardcoded requirement)
      const responseUsers = await fetch(`${derivedUrl}/rest/v1/users?select=count&limit=1`, {
        headers: { 'apikey': apiKey, 'Authorization': `Bearer ${apiKey}` }
      });

      if (responseReg.ok && responseUsers.ok) {
        setTestStatus('success');
        setTestMessage('Connected to both Registrations and Users tables!');
      } else if (!responseReg.ok) {
         setTestStatus('failed');
         setTestMessage(`Table '${tableName}' not accessible.`);
      } else if (!responseUsers.ok) {
         setTestStatus('failed');
         setTestMessage(`Table 'users' not accessible. Schema required.`);
      } else {
        setTestStatus('failed');
        setTestMessage('Connection refused.');
      }
    } catch (err) {
      setTestStatus('failed');
    ...
    <div className="space-y-6 animate-fade-in pb-10" onClick={() => setActiveMenuId(null)}>
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage external project connections.</p>
        </div>
        <button
      ...
      {apps.length === 0 && (
        <div className="text-center py-20 bg-white rounded-xl border border-dashed border-slate-300">
          ...
          <h3 className="text-lg font-medium text-slate-600">No Projects Connected</h3>
          <p className="text-slate-400 max-w-md mx-auto mt-2">Connect a Supabase project to automatically fetch 'registrations' and 'users'.</p>
          <button
             onClick={(e) => { e.stopPropagation(); openAddModal(); }}
        ...
        {apps.map((app) => (
          <div key={app.id} className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all p-6 flex flex-col justify-between group relative">
            ...
            <div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between text-xs text-slate-500">
                  <span className="flex items-center gap-1" title="Fetching from registrations and users tables">
                    <Layers className="h-3 w-3 text-indigo-500" /> Tables: {app.tableName} + users
                  </span>
            ...
            <div className="mt-6 pt-6 border-t border-slate-100 flex justify-between items-center">
              <span className="text-sm font-medium text-slate-600">{app.userCount} Records</span>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => setIsModalOpen(false)}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 transform transition-all scale-100" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-xl font-bold text-slate-800 mb-4">{isEditing ? 'Edit Connection' : 'Connect Project'}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                  ...
                  />
                  <p className="text-[10px] text-slate-400 mt-1">Copy "Host" from Supabase Settings.</p>
                </div>

                <div className="col-span-2 bg-amber-50 p-3 rounded-lg border border-amber-200 mt-2">
                   <div className="flex items-center gap-2 mb-2">
                      <Shield className="h-4 w-4 text-amber-700" />
                      <label className="block text-sm font-bold text-amber-900">Supabase API Key (Anon)</label>
                   </div>
                   <input
                    type="password"
                    required
                ...
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-slate-700 mb-1">Registration Table</label>
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <div className="text-xs text-slate-400 bg-slate-50 px-2 py-2 rounded border border-slate-200 whitespace-nowrap">
                       + "users" table
                    </div>
                  </div>
                  <p className="text-[10px] text-slate-400 mt-1">
                    System will automatically fetch from <strong>{tableName || 'registrations'}</strong> (Pending) AND <strong>users</strong> (Active).
                  </p>
                </div>
              </div>
              ...
              <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                <div className="flex items-center justify-between">
                   <span className="text-sm text-slate-600 font-medium">Connectivity Check</span>
                   <button
                    type="button"
                   ...
                   >
                     {isTesting ? 'Verifying...' : 'Test Dual-Table Sync'}
                   </button>
                </div>
